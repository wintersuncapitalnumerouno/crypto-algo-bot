# =============================================================
# live/executor_stochvol_v5_candidate.py — StochVol V5 Candidate — Wallet 1
# =============================================================
#
# V5 CANDIDATE: entry_window=4, vol_min_ratio=0.2
# All other params identical to V4 production.
# Stop: Fixed 2% (unchanged). Trail: unchanged.
#
# Runs StochVol V5-candidate strategy live on Hyperliquid.
...
class StochVolExecutor:
    ...
    def __init__(self):
        ...
        from live.signal_engine_v5_candidate import StochVolSignalEngine
        ...
        log("=" * 60)
        log("  StochVol V5-candidate Executor starting — Wallet 1")
    ...
    def _log_regime(self):
        """Log regime label. Observation only — no action taken."""
        try:
            from research.regime import classify_trend, classify_vol
            df_5m = self.feed.get_candles("ETH")
            ...
            if len(df_4h) < 210:
                return
            trend = classify_trend(df_4h).iloc[-1]
            vol = classify_vol(df_4h).iloc[-1]
            regime = f"{trend}_{vol.replace('_vol', '')}"
            log(f"  📊 Regime: {regime} (observation only)")
        except Exception as e:
            log(f"  ⚠️  Regime log skip: {e}")
