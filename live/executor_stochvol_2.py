# =============================================================
# live/executor_stochvol_2.py — StochVol V3 Live Executor — Wallet 1
# =============================================================
#
...
class StochVolExecutor:
    ...
    def __init__(self):
        ...
        log("=" * 60)
        log("  StochVol V3 Executor starting — Wallet 1")
    ...
    def _validate_env(self):
        if not PRIVATE_KEY or not WALLET_ADDRESS:
            log("❌ HL_WALLET1_PRIVATE_KEY or HL_WALLET1_WALLET_ADDRESS not set in .env")
            sys.exit(1)
    ...
    def _enter_trade(self, coin: str, signal: dict, equity: float):
        ...
        try:
            ...
            send_telegram(f"<b>StochVol WALLET 1 ENTRY — {coin} {direction_emoji} {direction.upper()}</b>\nPrice: {price:.6f} | Size: ${size_usd:.2f}\nStop: {sl:.6f} | Equity: ${equity:.2f}")
    ...
    def _exit_trade(self, coin: str, reason: str, current_price: float, candle_time: str):
        ...
        try:
            ...
            if True:
                ...
                send_telegram(f"<b>StochVol WALLET 1 EXIT — {coin} {pos.direction.upper()}</b>\nReason: {reason}\nPnL: {pnl_emoji} ${pnl_usd:+.2f} ({pnl_pct*100:+.2f}%)\nEquity: ${self.equity:.2f}")
    ...
    def run(self):
        log("🚀 StochVol V3 (wallet 1) executor running. Ctrl+C or pkill to stop.")
        send_telegram("🤖 <b>StochVol V3 — WALLET 1 started</b>\nServer: Legatus | Coins: " + ", ".join(COINS))
        while True:
            try:
                self.run_once()
